        ...
        <div className="md:hidden divide-y divide-slate-100">
          {filteredMessages.length === 0 ? (
            <div className="px-4 py-12 text-center text-slate-400">
              <MessageSquare className="w-10 h-10 mx-auto mb-2 opacity-30" />
              <p className="font-medium text-xs">No WhatsApp delivery records found</p>
              <p className="text-[11px] text-slate-400 mt-1">Notifications dispatched will appear here automatically</p>
            </div>
          ) : (
            filteredMessages.map((msg) => (
              <div key={msg.id} className="p-4 space-y-2.5 text-xs hover:bg-slate-50/70 transition-colors">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="font-mono font-bold text-slate-900 flex items-center gap-1.5 text-xs truncate">
                      <Phone className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                      <span className="break-all">{msg.recipientPhone}</span>
                    </div>
                    <div className="text-[11px] text-slate-500 mt-0.5 font-medium">
                      {msg.templateType} • <span className="font-mono text-slate-400">{msg.templateName}</span>
                    </div>
                  </div>
                ...
                {msg.messageContent && (
                  <p className="text-slate-600 text-xs bg-slate-50 p-2.5 rounded-lg border border-slate-100 break-words line-clamp-3">
                    {msg.messageContent}
                  </p>
                )}
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                  <div>
                    <span className="block text-[10px] text-slate-400 uppercase font-medium">Timestamp</span>
                    <span>{msg.sentAt ? new Date(msg.sentAt).toLocaleString() : new Date(msg.createdAt).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setSelectedMessage(msg)}
                      className="text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 px-2.5 py-1.5 rounded-lg transition-colors inline-flex items-center gap-1"
                    >
                      <Eye className="w-3 h-3" /> View
                    </button>
                    {(msg.status === 'FAILED' || msg.status === 'PENDING') && (
                      <button
                        onClick={() => handleResend(msg.id)}
                        disabled={resendingId === msg.id}
                        className="text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1.5 rounded-lg transition-colors inline-flex items-center gap-1 disabled:opacity-50"
                      >
                        <RotateCcw className={clsx('w-3 h-3', resendingId === msg.id && 'animate-spin')} />
                        Resend
                      </button>
